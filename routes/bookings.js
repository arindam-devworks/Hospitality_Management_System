...
router.post('/', (req, res) => {
    ...
    // Validate input
    if (!hotel_id || !checkin || !checkout || !guests || !total || !payment_method) {
    ...
    const query = `
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?)
    `;
    db.query(query, [id, hotel_id, checkin, checkout, guests, total, payment_method], (err, result) => {
        if (err) {
...
router.delete('/:id', (req, res) => {
    db.query('UPDATE bookings SET status = ? WHERE id = ?', ['cancelled', req.params.id], (err, result) => {
        if (err) {
