...
passport.use(new GoogleStrategy({
    ...
    const query = 'INSERT INTO users (fullname, email, googleId) VALUES (?, ?, ?) ON DUPLICATE KEY UPDATE fullname=VALUES(fullname), googleId=VALUES(googleId)';
...
passport.use(new FacebookStrategy({
    ...
    const query = 'INSERT INTO users (fullname, email, facebookId) VALUES (?, ?, ?) ON DUPLICATE KEY UPDATE fullname=VALUES(fullname), facebookId=VALUES(facebookId)';
...
app.post('/api/user/bookings', (req, res) => {
    ...
    db.query(query, [email], (err, results) => {
        if (err) return res.status(500).json({ message: 'Error fetching bookings', error: err.message });
        res.json(results);
    });
});
...
app.post('/api/user/checkin', (req, res) => {
    ...
    if (!bookingId || !email) return res.status(400).json({ message: 'Booking ID and email required' });
    const query = 'UPDATE bookings SET status = ? WHERE id = ? AND user_email = ?';
...
app.get('/auth-success', (req, res) => {
    if (req.user) {
        res.send(`<script>window.opener && window.opener.postMessage({ user: ${JSON.stringify(JSON.stringify(req.user))} }, '*'); window.close();</script>`);
    } else {
        res.redirect('/login');
    }
});
